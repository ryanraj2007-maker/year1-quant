...
class YFinanceProvider(DataProvider):
    def fetch(self, symbol: str, start: str, end: str, interval: str) -> pd.DataFrame:
        ...
        # yfinance limits the number of data points per request, so we may need to fetch in chunks
        ...
        # Download in chunks to stay within API limits
        chunks, s = [], start_dt
        while s < end_dt:
            e = min(s + pd.Timedelta(days=_chunk), end_dt)
            chunk = yf.download(symbol, 
                                start=s, 
                                end=e, 
                                interval=interval, 
        ...
        df.columns = [c.lower() for c in df.columns]  # Standardize column names to lowercase
        if df.index.tz is not None:
            df.index = df.index.tz_convert("America/New_York").tz_localize(None)  # Convert to tz-naive Eastern time
        return df
...
class DatabentoProvider(DataProvider):
    ...
    def fetch(self, symbol: str, start: str, end: str, interval: str) -> pd.DataFrame:
        ...
        db_symbol = f"{symbol}.c.0"  # Databento format for continuous futures contract
        ...
        df.index = df.index.tz_convert("America/New_York").tz_localize(None)  # Convert to tz-naive Eastern time
        df.columns = [c.lower() for c in df.columns]  # Standardize column

        for col in ["open", "high", "low", "close"]:
                df[col] = df[col] / 1e9 # Convert from nanodollars to dollars
